 ...
 *
 * The line-level fix worked perfectly. This script covers the Global Discounts
 * attached to the Invoice Header, ensuring `globalDiscountType='PERCENT'` is enabled.
 */
...
function main() {
    return __awaiter(this, void 0, void 0, function* () {
        console.log('\n══════════════════════════════════════════════');
        console.log('  🔄 PATCH: Restoring GLOBAL Discount Types');
        console.log('══════════════════════════════════════════════\n');
        ...
        try {
                ...
                try {
                    ...
                    for (const h of headers) {
                        ...
                        if (discountTypeParam == 2) {
                            ...
                            // When it's a percent, the percentage value is exactly what `invDiscount` holds
                        ...
                        // Forcefully update the DB
                        updateValues.push([sqlDiscountType, sqlDiscountValue, invoiceId]);
            ...
            console.log(`\n  ✅ Successfully updated ${updatedInvoicesCount} invoices with correct GLOBAL discounting metadata.`);
...
function updateChunk(conn, batchArgs) {
    return __awaiter(this, void 0, void 0, function* () {
        const promises = batchArgs.map(args => {
            return conn.query(`UPDATE invoices SET globalDiscountType = ?, globalDiscount = ? WHERE id = ?`, [args[0], args[1], args[2]]);
        });
